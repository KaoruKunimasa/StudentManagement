...
/**
 * 受講生情報を取り扱うサービスです。 受講生情報の検索や登録・更新処理を行います。
 */
@Service
public class StudentService {
  ...
  /**
   * 受講生詳細の一覧検索です。 全件検索を行うので、全件検索は行いません。
   *
   * @return　受講生詳細一覧（全件）
   */
  ...
  public StudentDetail searchStudent(String id) {
    Student student = repository.saarchStudent(id);
    List<StudentCourse> studentCourse = repository.searchStudentCourse(student.getId());
    return new StudentDetail(student, studentCourse);
  }

  /**
   * 受講生詳細の登録を行います。受講生と受講生コース情報を個別に登録し、受講生コース情報には受講生情報と紐づける値とコース開始日、コース終了日を設定します。
   *
